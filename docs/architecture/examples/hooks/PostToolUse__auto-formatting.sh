...
HOOK_NAME="PostToolUse__auto-formatting"
...
get_metrics_collector().record_counter('hook.auto_format.unsupported_type', tags={'hook': '$HOOK_NAME', 'extension': '$EXTENSION'})
" 2>/dev/null || true
        call_observability "end" "true" || true
        exit 0
        ;;
...
if [[ -z "$FORMATTER" ]]; then
    ...
    # Track missing formatter
    python3 -c "
import sys
sys.path.insert(0, '$HOOK_DIR/../../../')
from observability.metrics import get_metrics_collector
get_metrics_collector().record_counter('hook.auto_format.formatter_missing', tags={'hook': '$HOOK_NAME', 'formatter': '$FORMATTER'})
" 2>/dev/null || true
    call_observability "end" "true" || true
    exit 0
fi
...
if "$FORMATTER" "${FORMATTER_ARGS[@]}" 2>&1 | head -20; then
    ...
    if [[ "$ORIGINAL_HASH" != "$NEW_HASH" ]]; then
        ...
        # Track file formatted
        python3 -c "
import sys
sys.path.insert(0, '$HOOK_DIR/../../../')
from observability.metrics import get_metrics_collector
get_metrics_collector().record_counter('hook.auto_format.files_formatted', tags={'hook': '$HOOK_NAME', 'formatter': '$FORMATTER', 'extension': '$EXTENSION'})
" 2>/dev/null || true
    else
        log_info "No changes needed for $FILENAME"
        FORMAT_SUCCESS=1

        # Track file unchanged
        python3 -c "
import sys
sys.path.insert(0, '$HOOK_DIR/../../../')
from observability.metrics import get_metrics_collector
get_metrics_collector().record_counter('hook.auto_format.files_unchanged', tags={'hook': '$HOOK_NAME', 'formatter': '$FORMATTER', 'extension': '$EXTENSION'})
" 2>/dev/null || true
    fi
else
    log_error "Formatting failed for $FILENAME"

    # Track formatting failure
    python3 -c "
import sys
sys.path.insert(0, '$HOOK_DIR/../../../')
...
logger.error('Formatting failed', hook='$HOOK_NAME', formatter='$FORMATTER', file='$FILENAME')
get_metrics_collector().record_counter('hook.auto_format.format_failed', tags={'hook': '$HOOK_NAME', 'formatter': '$FORMATTER', 'extension': '$EXTENSION'})
" 2>/dev/null || true
...
sys.path.insert(0, '$HOOK_DIR/../../../')
from observability.metrics import get_metrics_collector
get_metrics_collector().record_histogram('hook.auto_format.duration.seconds', $DURATION, tags={'hook': '$HOOK_NAME', 'success': 'true', 'formatter': '$FORMATTER'})
" 2>/dev/null || true
